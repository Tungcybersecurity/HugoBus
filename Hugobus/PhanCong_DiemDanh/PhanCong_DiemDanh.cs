...
namespace GUI.PhanCong_DiemDanh
{
    public partial class PhanCong_DiemDanh : Form
    {
        ...
        int nhanThem = 0;
        public string NgayTemp = string.Empty;
        ...
        public void loadForm()
        {
            ...
            dateTimePickerNgayDiemDanh.MinDate = DateTime.Now;
        ...
        private void phancongTable_CellMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            try
            {
                ...
                if (dong >= 0)
                {
                    string ngay = phancongTable.Rows[dong].Cells["Ngày đưa đón"].Value.ToString();
                    DateTime selectedDate = DateTime.Parse(ngay);
                    if (selectedDate.Date < DateTime.Now.Date)
                    {
                        buttonDiemDanh.Enabled = false;
                        
                    }
                    else
                    {
                        buttonDiemDanh.Enabled = true;
                        
                    }
                }
                idPhanCong = phancongTable.Rows[dong].Cells[0].Value.ToString();
                NgayTemp = phancongTable.Rows[dong].Cells["Ngày đưa đón"].ToString();
        ...
        {
            if (NgayTemp != DateTime.Today.ToString("yyyy-MM-dd"))
            {
                MessageBox.Show("Không thể điểm danh cho ngày này");
                return;
            }
        ...
        {
            if (idHopDong_CPC == string.Empty)
            {
                MessageBox.Show("Vui lòng chọn 1 hàng dữ liệu trong bảng CHƯA PHÂN CÔNG");
                return;
            }
            else
            {
                int kq = new PhanCongBUS().xoaPhanCong2(idHopDong_CPC);
                if (kq > 0)
                {
                    MessageBox.Show("Xóa thành công");
                    string loai = "1";
                    string TenThongBao = "Hợp đồng: ID = " + idHopDong_CPC + ", trạng thái: Không đủ điều kiện";
                    int tbhd = new ThongBaoBUS().themThongBaoHopDong(TenThongBao, loai, idHopDong_CPC);
                    if (tbhd > 0)
                    {
